...
const StyledDatePicker = styled(DatePicker)`
  &.ant-picker {
    ...
    border-radius: 8px;
    border: 1px solid
      ${(props) => props.theme?.token?.borderColorBase || "#d9d9d9"};
...
const CrearEditarEmpleado = ({ visible, onClose, empleadoData, isEditing }) => {
  ...
  const fetchProvincias = async () => {
    try {
      ...
      notification.error(texts.updateError, texts.updateErrorDesc);
    } catch (error) {
  ...
  const fetchMunicipios = async (provinciaId) => {
    try {
      const response = await axios.get(
        `${api_url}/api/municipio?idProvincia=${provinciaId}`,
        {
          headers: {
            Authorization: `Bearer ${getAccessToken()}`,
          },
  ...
  const handleSubmit = async (values) => {
    ...
    const formattedData = {
      ...
      fechaNacimiento:
        values.fechaNacimiento && dayjs(values.fechaNacimiento).isValid()
    ...
    if (isEditing) {
      ...
      const changedFields = {};
      ["nombres", "apellidos", "correo", "estado", "idTipoUsuario"].forEach(
        (field) => {
      ...
      if (
        ...
        notification.info(texts.validationError, texts.validationErrorDesc);
        return;
      }
      ...
      try {
        ...
        const requestBody = {
          ...
          datosPersonales:
            Object.keys(changedPersonaFields).length > 0
              ? changedPersonaFields
              : undefined, // Solo si hay cambios en datosPersonales
        };
        ...
        notification.success(texts.userUpdated, texts.userUpdatedDesc);
        handleClose();
      } catch (error) {
          ...
          <FormContainer>
            ...
            <FormRow>
              <Form.Item
                ...
                rules={[
                  {
                    required: true,
                    message: texts.enterIdNumber || "Enter your ID",
                  },
                  {
                    pattern: /^\d{3}-\d{7}-\d{1}$/,
            ...
            <FormRow>
              <Form.Item
                ...
                rules={[
                  {
                    required: true,
                    message: texts.gender || "Please select your gender",
                  },
                ]}
              ...
              >
                <Select
                  placeholder={
                    texts.maritalStatus || "Select your marital status"
                  }
                  style={{ width: "100%", height: "44px" }}
                >
                  <Select.Option value="soltero">
                    {language === "es" ? "Soltero/a" : "Single"}
                  </Select.Option>
                  <Select.Option value="casado">
                    {language === "es" ? "Casado/a" : "Married"}
                  </Select.Option>
                  <Select.Option value="divorciado">
                    {language === "es" ? "Divorciado/a" : "Divorced"}
                  </Select.Option>
                  <Select.Option value="viudo">
                    {language === "es" ? "Viudo/a" : "Widowed"}
                  </Select.Option>
                </Select>
              </Form.Item>
            </FormRow>
                    ...
                    ({ getFieldValue }) => ({
                      validator(_, value) {
                        ...
                        return Promise.reject(
                          new Error(texts.confirmPasswordError),
                        );
                      },
                    }),
                  ]}
            ...
            <FormRow>
              <Form.Item
                ...
                rules={[
                  {
                    required: true,
                    message: texts.enterPhone || "Enter your phone number",
                  },
                  {
                    pattern: /^\d{3}-\d{3}-\d{4}$/,
              ...
              <Form.Item
                ...
                initialValue={
                  empleadoData?.datosPersonales?.fechaNacimiento
                    ? dayjs(empleadoData.datosPersonales.fechaNacimiento)
                    : null
                }
              >
                <DatePicker
            ...
            <FormRow>
              <Form.Item
                ...
                initialValue={
                  empleadoData?.datosPersonales?.ubicacion?.provincia?.id
                }
              >
                <Select
              ...
              <Form.Item
                ...
                initialValue={
                  empleadoData?.datosPersonales?.ubicacion?.municipio?.id
                }
              >
                <Select
                  ...
                  disabled={
                    empleadoData?.datosPersonales?.ubicacion?.municipio?.id
                      ? false
                      : !selectedProvincia
                  } // Esta línea ya deshabilita el select
            ...
            <Form.Item
              ...
              initialValue={
                empleadoData &&
                `${empleadoData?.datosPersonales?.ubicacion?.direccion}`
              }
            >
            ...
            >
              ...
              >
                {cargos.map((cargo) => (
                  <Select.Option
                    key={cargo.idtipopersona}
                    value={cargo.idtipopersona}
                    label={cargo.nombre}
                  >
                    {cargo.nombre}
                  </Select.Option>
            ...
            >
              ...
              >
                ...
                <Select.Option value="deshabilitado">
                  {texts.disabled}
                </Select.Option>
              </Select>
            </Form.Item>
